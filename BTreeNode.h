...
class BTLeafNode {
  ...
  private:
    ...
    */
    unsigned char buffer[PageFile::PAGE_SIZE];
	int num_keys;						//	the number of keys in the node
	PageId next_leaf;					//	page id of the next leaf
}; 
...
class BTNonLeafNode {
  ...
  private:
    ...
    */
    unsigned char buffer[PageFile::PAGE_SIZE];
	int root;							//	check if node is a root
	int left;							//	check if the node has a left pointer
	int num_keys;						//	the number of keys in the node
}; 
